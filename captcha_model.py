...
# https://github.com/ypwhs/captcha_break/blob/master/ctc_pytorch.ipynb
...
def valid(model, optimizer, epoch, dataloader, *, callback=None):
    model.eval()
    with tqdm(dataloader) as pbar, torch.no_grad():
        ...
        for batch_index, (data, target, input_lengths, target_lengths) in enumerate(pbar):
            ...
            if batch_index == len(pbar)-1:
                if callback is not None:
                    extra_desc = callback(model)
                else:
                    extra_desc = ""
                pbar.set_description(f'Test : {epoch} Loss: {loss_mean:.4f} Acc: {acc_mean:.4f} {extra_desc}')
            else:
